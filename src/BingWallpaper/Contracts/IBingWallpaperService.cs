...
namespace BingWallpaper.Contracts
{
	public interface IBingWallpaperService
	{
		...
		/// <returns></returns>
		Task DownloadAsync(string imageUrl, string imageName, string imageExtension, string localDownloadPath, string folderPrefix = "Bing/");

		/// <summary>
		/// This method will return all the information releated to the daily bing wallpaper, so that one may download it.
		/// </summary>
